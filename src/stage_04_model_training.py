...
def main(config_path, params_path):
    ...
    train_data_matrix = joblib.load(featurized_train_data_path)
    labels = np.squeeze(train_data_matrix[:,1].toarray())
    X = train_data_matrix[:,2]
    
    logging.info(f"Input Training Matrix size {train_data_matrix.shape}")
    logging.info(f"Labels size {labels.shape}")
    logging.info(f"Training Data   size {X.shape}")
